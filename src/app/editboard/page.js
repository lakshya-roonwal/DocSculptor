...
const Artboard = () => {
  const [textElements, setTextElements] = useState([
    ...
    },
    {
        ...
        "fontSize": 16,
        "isBold": false,
        "content": "Hello dear , {{Name}} this is to inform you that there will be a small party  at lakshy's house "
    },
    {
        "id": 1699983540034,
        "x": 16,
        ...
        <textarea type="text" 
          ...
          <div className="text-properties">
            ...
            <div className="text-styling">
              <span>Bold</span>
              <input type="checkbox" name="" id="" value={textElement.isBold} onChange={(e) => handleTextBoldChange(e, textElement)}/>

              <span>Font Size</span>
              <input type="number" value={textElement.fontSize} onChange={(e) => handleTextFontChange(e, textElement)}/>
            </div>
          </div>
      </div>
    ))}
      </div>
      <button className="mx-2 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onClick={addText}>Add Text</button>
      <Link className="mx-2 py-2.5 px-5 me-2 mb-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700" 
      href={"/renderboard"}
      >
      Render
      </Link>
    </div>
    <div className="relative overflow-hidden border-2 border-black" id="artboard" style={{width:"210mm",height:"297mm"}} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp}>
      {textElements.map((textElement) => (
        <div key={textElement.id}
          className="text-element absolute cursor-move whitespace-nowrap"
          style={{ left: textElement.x, top: textElement.y,maxWidth:"210mm"}}
